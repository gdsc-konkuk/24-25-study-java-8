...
public class OrderProcessingService {
    ...
    public List<Order> findHighValueOrders(double minTotal) {
        return orders.stream()
                .filter(order -> order.getProducts().stream().mapToDouble(Product::getPrice).sum() >= minTotal).toList();
    }

    // 각 고객별 총 주문 금액을 계산합니다.
    public Map<String, Double> calculateTotalOrderValuePerCustomer() {
        return orders.stream()
                .collect(Collectors.groupingBy(Order::getCustomerId, Collectors.summingDouble(order-> order.getProducts().stream().mapToDouble(Product::getPrice).sum())));
    }

...
public class LibraryService {
    ...
    public Map<String, Long> countBooksByCategory() {
        return books.stream().flatMap(book->book.getCategories().stream())
    ...
    public double getAverageBookAge() {
        return books.stream().mapToLong(book -> ChronoUnit.YEARS.between(book.getPublishDate(), LocalDate.now()))
                .average().orElse(0);
    }
    ...
    public List<Book> getRecentBooks(int n) {
        return books.stream().sorted(Comparator.comparing(Book::getPublishDate).reversed())
                .limit(n).toList();
    }
    ...
    public boolean lendBook(String isbn) {
        ...
        return book.filter(Book::isAvailable).map(b ->{
    ...
    public int getTotalTitleLength() {
        return books.stream().map(Book::getTitle).map(String::length).reduce(0, Integer::sum);
    }
    ...
    public List<Book> getValidBooks(BookValidator validator) {
        return books.stream().filter(validator::validate).collect(Collectors.toList());
    }

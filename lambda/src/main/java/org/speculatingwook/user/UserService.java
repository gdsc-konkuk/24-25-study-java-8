...
public class UserService {
    ...
    public List<User> getUsersSortedByAge() {
        return this.users.stream().sorted(Comparator.comparingInt(User::getAge)).toList();
    }
    ...
    public List<User> getUsersOver30() {
        Predicate<User> agePredicate = user -> user.getAge() >= 30;
        return this.users.stream().filter(agePredicate).toList();
    }
    ...
    public double getAverageSalary() {
        return users.stream().collect(Collectors.summingDouble(User::getSalary))/users.size();
    }
    ...
    public List<String> getUpperCaseNamesOfUsersAboveAge(int age) {
        return users.stream().filter(user -> user.getAge() > age).map(User::getName).map(String::toUpperCase).toList();
    }
    ...
    public <R> List<R> mapUsers(Function<User, R> mapper) {
        return users.stream().map(mapper).toList();
    }
    ...
    public Map<String, Double> getAverageAgeByDepartment() {
        return users.stream().collect(Collectors.groupingBy(User::getDepartment, Collectors.averagingInt(User::getAge)));
    }
    ...
    public List<Map.Entry<String, Double>> getDepartmentsSortedByAverageAge() {
        return users.stream()
                .collect(Collectors.groupingBy(User::getDepartment, Collectors.averagingInt(User::getAge)))
                .entrySet().stream()
